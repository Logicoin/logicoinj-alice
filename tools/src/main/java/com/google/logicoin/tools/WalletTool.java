...
import com.google.logicoin.store.*;
    ...
    private static void send(List<String> outputs, BigInteger fee, String lockTimeStr, boolean allowUnconfirmed) throws VerificationException {
        try {
            ...
            if (!wallet.completeTx(req)) {
                System.err.println("Insufficient funds: have " + Utils.logicoinValueToFriendlyString(wallet.getBalance()));
                return;
            }
            try {
                if (lockTimeStr != null) {
                ...
                wallet.addEventListener(new AbstractWalletEventListener() {
                    ...
                    public synchronized void onChange() {
                        ...
                        if (condition.matchBitcoins(balance)) {
                            System.out.println(Utils.logicoinValueToFriendlyString(balance));
                            latch.countDown();
                        }
                    }
                });
