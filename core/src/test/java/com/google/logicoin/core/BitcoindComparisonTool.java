...
import com.google.logicoin.params.RegTestParams;
...
import com.google.logicoin.utils.BriefLogFormatter;
...
/**
 * A tool for comparing the blocks which are accepted/rejected by logicoind/logicoinj
 * It is designed to run as a testnet-in-a-box network between a single logicoind node and logicoinj
...
public class BitcoindComparisonTool {
    ...
    private static Sha256Hash logicoindChainHead;
    private static volatile Peer logicoind;
    private static volatile InventoryMessage mostRecentInv = null;

    public static void main(String[] args) throws Exception {
        BriefLogFormatter.init();
        System.out.println("USAGE: logicoinjBlockStoreLocation runLargeReorgs(1/0) [port=18444]");
        ...
        // logicoind MUST be on localhost or we will get banned as a DoSer
        ...
        peers.addEventListener(new AbstractPeerEventListener() {
            ...
            public void onPeerDisconnected(Peer peer, int peerCount) {
                ...
                log.error("logicoind node disconnected!");
                System.exit(1);
            }
            ...
            public Message onPreMessageReceived(Peer peer, Message m) {
                if (m instanceof HeadersMessage) {
                    for (Block block : ((HeadersMessage) m).getBlockHeaders())
                        logicoindChainHead = block.getHash();
                    return null;
                } else if (m instanceof Block) {
                    log.error("logicoind sent us a block it already had, make sure logicoind has no blocks!");
                    System.exit(1);
        ...
        logicoindChainHead = params.getGenesisBlock().getHash();
        
        // Connect to logicoind and make sure it has no blocks
        peers.start();
        peers.setMaxConnections(1);
        peers.downloadBlockChain();
        
        while (logicoind == null)
            Thread.sleep(50);
        ...
        for (Rule rule : blockList.list) {
            if (rule instanceof BlockAndValidity) {
                ...
                logicoind.sendMessage(message);
                // logicoind doesn't request blocks inline so we can't rely on a ping for synchronization
                for (int i = 0; !blocksRequested.contains(nextBlock.getHash()); i++) {
                    if (i % 20 == 19)
                        log.error("logicoind still hasn't requested block " + block.ruleName + " with hash " + nextBlock.getHash());
                    Thread.sleep(50);
                }
                logicoind.sendMessage(nextBlock);
                locator.clear();
                locator.add(logicoindChainHead);
                logicoind.sendMessage(new GetHeadersMessage(params, locator, hashTo));
                logicoind.ping().get();
                if (!chain.getChainHead().getHeader().getHash().equals(logicoindChainHead)) {
                    differingBlocks++;
                    log.error("logicoind and logicoinj acceptance differs on block \"" + block.ruleName + "\"");
                }
                ...
                if (mostRecentInv == null && !((MemoryPoolState) rule).mempool.isEmpty()) {
                    log.error("logicoind had an empty mempool, but we expected some transactions on rule " + rule.ruleName);
                    mempoolRulesFailed++;
                } else if (mostRecentInv != null && ((MemoryPoolState) rule).mempool.isEmpty()) {
                    log.error("logicoind had a non-empty mempool, but we expected an empty one on rule " + rule.ruleName);
                    mempoolRulesFailed++;
                    ...
                    if (matches)
                        continue;
                    log.error("logicoind's mempool didn't match what we were expecting on rule " + rule.ruleName);
                    log.info("  logicoind's mempool was: ");
                    for (InventoryItem item : mostRecentInv.items)
                        log.info("    " + item.hash);
        ...
        log.info("Done testing.\n" +
        		"Blocks which were not handled the same between logicoind/logicoinj: " + differingBlocks + "\n" +
