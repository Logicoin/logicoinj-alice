 ...
 * the lightweight SPV mode, a {@link com.google.logicoin.store.SPVBlockStore} is the right choice.</p>
 *
 ...
 * {@link com.google.logicoin.store.H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
public abstract class AbstractBlockChain {
    ...
    private boolean add(Block block, boolean tryConnecting,
        ...
        try {
            ...
            // article here for more details: http://code.google.com/p/logicoinj/wiki/SecurityModel
            try {
                block.verifyHeader();
        ...
        for (Transaction tx : transactions) {
            try {
                if (listener != null && listener.isTransactionRelevant(tx)) {
                    if (clone)
                        tx = new Transaction(tx.params, tx.logicoinSerialize());
    ...
    private void checkTestnetDifficulty(StoredBlock storedPrev, Block prev, Block next) throws VerificationException, BlockStoreException {
        ...
        // There is an integer underflow bug in logicoin-qt that means mindiff blocks are accepted when time
        // goes backwards.
