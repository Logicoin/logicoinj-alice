...
import com.google.logicoin.crypto.TransactionSignature;
import com.google.logicoin.script.Script;
import com.google.logicoin.script.ScriptBuilder;
import com.google.logicoin.script.ScriptOpCodes;
 ...
 * <a href="http://code.google.com/p/logicoinj/wiki/WorkingWithTransactions">"Working with transactions"</a> in the
...
public class Transaction extends ChildMessage implements Serializable, IsMultiBitClass {
     ...
     */
    public static final BigInteger REFERENCE_DEFAULT_MIN_TX_FEE = CoinDefinition.DEFAULT_MIN_TX_FEE;
     ...
     */
    public static final BigInteger MIN_NONDUST_OUTPUT = CoinDefinition.DUST_LIMIT;

    // These are serialized in both logicoin and java serialization.
    ...
    public String toString(AbstractBlockChain chain) {
        ...
        for (TransactionOutput out : outputs) {
            ...
            try {
                ...
                s.append(" ");
                s.append(logicoinValueToFriendlyString(out.getValue()));
    ...
    public void clearInputs() {
        ...
        this.length = this.logicoinSerialize().length;
    }
     ...
     * a wrapper around calling {@link Transaction#hashForSignature(int, byte[], com.google.logicoin.core.Transaction.SigHash, boolean)}
    ...
    public synchronized Sha256Hash hashForSignature(int inputIndex, byte[] connectedScript, byte sigHashType) {
        ...
        //
        //   https://en.logicoin.it/wiki/Contracts
    ...
    @Override
    protected void logicoinSerializeToStream(OutputStream stream) throws IOException {
        uint32ToByteStreamLE(version, stream);
        stream.write(new VarInt(inputs.size()).encode());
        for (TransactionInput in : inputs)
            in.logicoinSerialize(stream);
        stream.write(new VarInt(outputs.size()).encode());
        for (TransactionOutput out : outputs)
            out.logicoinSerialize(stream);
     ...
     * This is useful in certain types of <a href="http://en.logicoin.it/wiki/Contracts">contracts</a>, such as
