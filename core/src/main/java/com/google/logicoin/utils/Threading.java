...
package com.google.logicoin.utils;
 ...
 * logicoinj performs cycle detection or not. Cycle detection is useful to detect bugs but comes with a small cost.
...
public class Threading {
     ...
     * logicoinj internally shouldn't normally run here, although currently there are a few exceptions.
     */
     ...
     * logicoinj library code is run, setting it after you started network traffic and other forms of processing
    ...
    static {
        ...
        // instantiating any logicoinj objects. The policy change will take effect only on new objects
        ...
        USER_THREAD = Executors.newSingleThreadExecutor(new ThreadFactory() {
            @Nonnull @Override public Thread newThread(@Nonnull Runnable runnable) {
                ...
                t.setName("logicoinj user thread");
                t.setDaemon(true);
